function newFastHNBF(init_delta, usr_zeros, itm_zeros)
    ...
    G_matGamma_Shp = bsxfun(@plus, init_delta*1000 * rand(M, K), 0.9*h_mu);
    G_matGamma_Rte = bsxfun(@plus, init_delta*1000 * rand(M, K), h_mu);
    G_matGamma = G_matGamma_Shp ./ G_matGamma_Rte;
    G_matGamma_Shp(usr_zeros,:) = 0;
    G_matGamma_Rte(usr_zeros,:) = 0;
    G_matGamma(usr_zeros,:) = 0;
    
    G_matDelta_Shp = bsxfun(@plus, init_delta*1000 * rand(N, K), 0.9*h_pi);
    G_matDelta_Rte = bsxfun(@plus, init_delta*1000 * rand(N, K), h_pi);
